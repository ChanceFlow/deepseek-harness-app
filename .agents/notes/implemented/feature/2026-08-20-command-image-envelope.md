...
The live host's command registry advertises attachment acceptance on
`plan` and `goal` (`input.attachments: true` in `commands/list`), and
`commands/execute` carries a base64 `submittedAttachments` arg with
host-side admission (`dsh-attachment` `admitEncodedImages` behind the
`admitCommandAttachments` mix). The app always sent an empty attachment
slot and cleared the composer's pending images on success — a
...
refusing. The pinned reference submodule then predated this surface (its
`CommandInputDescriptor` had only `hint`); the running host's installed
...
them first`. A claim that accepts submits with the attachments
(`submittedAttachments`); a handler error on a submission
...
- Adapter (`harness_repository_impl.dart`): `executeCommand` encodes
  each image as `{type: 'image', mediaType, data, name?}` under the
  `submittedAttachments` arg, in submission order — the shape the host's
  typert descriptor validates (the tag plus the mediaType literals
  png/jpeg/webp/gif, canonical base64) and `admitCommandAttachments`
  decodes; the app's admission limits use the same four media types.
  The descriptor is strict in both directions: a missing
  `submittedAttachments` and an unexpected sibling key each reject the
  call as `gateway/arguments-invalid` before any handler runs.
...
The wire names moved after that verification: the host's
2026-09-01 attachment admission rework renamed the arg from `images` to
`submittedAttachments` and tagged each entry, and renamed the roster
flag from `input.images` to `input.attachments`. The pinned submodule
carries the new names, but its own `client/connection` fixture still
forwards `args.images`, so a fixture transcribed from the reference
client keeps the stale key and passes while the live host refuses every
dispatch. The generated typert descriptor
(`dsh-commands/lib/typert.remote-client.js`, from the `@Remote`
signature in `packages/interaction/commands/src/index.ts`) is the
authority when the reference tree disagrees with itself.
...
the attachments encode as `{type: 'image', mediaType, data, name?}` under
`submittedAttachments`, in submission order.
