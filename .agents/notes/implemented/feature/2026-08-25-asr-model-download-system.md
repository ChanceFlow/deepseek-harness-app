...
- Resilient download engine `AsrDownloader` supporting HTTP Range resumption (across app runs and within a run: bounded automatic retries that resume from the partial file after a mid-body connection drop or a stalled stream, see the [mid-body retry fix](../bug-fix/2026-09-03-asr-download-mid-body-retry-resume.md)), single-concurrency queueing, disk space pre-flight validation backed by the Android `StatFs` method channel (requiring at least 1.3x model size free space; probe injected from app DI), cellular network guard, and temporary `.downloading` file staging with live SHA-256 verification.
