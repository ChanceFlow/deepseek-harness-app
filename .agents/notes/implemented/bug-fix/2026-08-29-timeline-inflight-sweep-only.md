...
slot, and in the running command row. Each violated the row contract:
the web `ToolRow` keeps its icon with a row sweep, and the web turn carries
one `turnStatus` line (shimmer label, elapsed clock past 15s) instead of
...
2. Call sites repeated `SizedBox(12)` in three colors without rationale.
3. The leading slot jumped geometry: 12px spinner running, 14px check/cross after.
4. 12px at 2px stroke is below M3 small-progress spec; the arc reads as mud.
5. Reduce-motion stopped sweeps but not spinners.
6. A round trip could show user-bubble and assistant loaders simultaneously.
...
- `activity_dot.dart` — `ActivityDot`: a static 8px `onSurfaceVariant`
  ...
  component and the slot wears the same geometry as `Icon(size: 14)`
  (same idiom as the rows' 2px dots).
...
- `turn_status_row.dart` — `TurnStatusRow`: the web `turnStatus` port.
  ...
  bespoke-motion reason) with a true text shimmer glint (`ShaderMask`
  with `BlendMode.srcATop` sweeping `scheme.statusGlint` across the
  letters, matching the web `background-clip: text` 1800ms shimmer) rather
  than a row-level wash, joining an elapsed clock (`formatJobDuration`)
  once the wait passes 15s, anchored at mount (the web fallback when the
  turn boundary sits outside the loaded window).
  ...
  below it. It rides both transcript modes: an extra tail item in flow,
  an extra sliver in outline. The letter-split sits behind
  `Semantics(excludeSemantics: true)` on the run-state label.
...
- **One shared polished spinner** (fixed size, one color, reduce-motion
  aware): rejected — still stacks rotation onto the sweep glare, one
  row with two infinite animations, and keeps the transcript's cheap
  look; the reference has no in-row spinner to port.
- **Keeping the 2×18 caret from the empty-text state**: rejected — a
  blinking caret with nothing to append reads as a rendering bug, with no
...
- **Row-level sweep wash without letter shimmer** (the message-row
  glare): rejected after user clarification — an `onSurface 0.2`
  background sweep reads as an unanchored wash across the row rather
  than the web reference's `background-clip: text` glint; the status line
  combines the letter hop with true text-color shimmer (`ShaderMask`).
- **Shimmer-only without letter hop**: rejected — a glare band across
  a static short line reads as flicker at phone scale without the
  letters' beat.
...
- A running turn costs one sweep per active row plus one status line
  (hop and shimmer tickers), replacing two tickers per row plus four
  ad-hoc spinners.
